...
// func executeMain2(s string) int {
// 	return 5
// }
...
func generateCombinations_(current []string, length int, result *[][]string) {
	if len(current) == length {
		// Make a copy of current to avoid slice referencing issues.
	...
	// For each element in the set [0, 1], append it to the current combination and recurse.
	for _, elem := range []string{".", "#"} {

		next := append(current, elem) // Append the current element to the combination.
		// fmt.Println(next)
		generateCombinations_(next, length, result)
	}
}
